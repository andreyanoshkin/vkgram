...
@IBDesignable @objc open class VkSwitch: UIControl {
    ...
    */
    @IBInspectable open var inactiveColor: UIColor = (UIColor.gray.withAlphaComponent(0.6)) {
        willSet {
            if !self.on && !self.isTracking {
                backgroundView.backgroundColor = newValue
            }
    ...
    *   Defaults to green.
    */
    @IBInspectable open var onTintColor: UIColor = UIColor.black.withAlphaComponent(0.9) {
        willSet {
            if self.on && !self.isTracking {
                backgroundView.backgroundColor = newValue
